   ...
   <div class="container">
      ...
      <form action="" method="post">
         <div class="flex-btn">
            ...
            <button type="button" id="generate-intents" class="option-btn">Tích hợp nội dung</button>
...
$(document).ready(function() {
    $('#generate-intents').click(function() {
        // Prepare data for the API call
        const title = 'Sample Title';  // Replace with actual title
        const summary = 'Sample Summary';  // Replace with actual summary
        const output_file = 'intents.json';  // Path to the intents file
        
        $.ajax({
            url: 'http://127.0.0.1:5000/generate_intents',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({ title: title, summary: summary, output_file: output_file }),
            success: function(response) {
                alert('Intents generated successfully!');
                console.log(response);
                
                // Optionally trigger the model training
                $.ajax({
                    url: 'http://127.0.0.1:5000/train_model',
                    type: 'POST',
                    contentType: 'application/json',
                    data: JSON.stringify({ json_file_path: output_file, namespace: 'default' }),  // Adjust namespace as needed
                    success: function(response) {
            ...
            error: function(error) {
                alert('Error generating intents');
                console.log(error);
            }
        });
    });
